...
public:
    ...
    Table(const string &name, const string &field_info, int default_degree);

    Table(const string &name, int default_degree);
...
class BTree {
public:
    BTree(const string &index_name, int degree, int page_size, FieldType type, int key_field_len,
          shared_ptr<DataPageMgr> data_page_mgr, TableOption *table_option);
...
class BTreeNode {
public:
    BTreeNode(long id, int degree, int page_size, int key_field_len, bool is_root, bool is_leaf);
