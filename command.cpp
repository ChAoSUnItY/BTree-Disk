...
const regex regexes[9] = {
        regex("^\\s*(create)\\s*table\\s*(\\w+)\\s*$"),
        regex("^\\s*(use)\\s*table\\s*(\\w+)\\s*$"),
        regex("^\\s*(delete)\\s*table\\s*(\\w+)\\s*$"),
        regex("^\\s*(clear_b)\\s*table\\s*(\\w+)\\s*$"),
...
void parseCommand(string input) {
    ...
    for (int i = 0; i < 7; i++) {
        ...
        if (firstMatch == "create") {
            ...
            cout << "Name: " << name << endl;
            db->useTable(match);
        } else if (firstMatch == "delete") {
            auto name = match.str(2);
            cout << "Delete command" << endl;
            cout << "Name: " << name << endl;
        } else if (firstMatch == "clear_b") {
            auto name = match.str(2);
            cout << "Clear_b command" << endl;
            cout << "Name: " << name << endl;
